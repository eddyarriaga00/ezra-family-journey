...
export function HomeScreen({ language, entries, onAdd, setScreen }: { language: Language, entries: Entry[], onAdd: (kind?: EntryKind) => void, setScreen: (screen: Screen) => void }) {
  ...
  return <div className="screen home-screen">
    <section className="home-hero">
      <div className="hero-photo"><img src="./images/ezra-home-placeholder.png" alt="Temporary newborn photo placeholder"/></div>
    ...
    <section className="latest-section"><header><h2>{t.latestMoments}</h2><Sprout aria-hidden="true"/></header>
      <button className="moment-preview warm" onClick={() => setScreen('moments')}><img src="./images/welcome-home-placeholder.png" alt="Welcome home keepsake placeholder"/><span><small><Heart/> {formatDate(milestones[1]?.date || '2026-05-28', language)}</small><strong>{t.welcomeHome}</strong></span><ChevronRight/></button>
